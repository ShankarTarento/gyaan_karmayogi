...
class TextSearchResultScreenState extends State<TextSearchResultScreen>
  ...
  void initState() {
    ...
    _textController.text = widget.searchKeyword;
    _searchText = widget.searchKeyword;
    _getCourses();
    _getUsersByText();
  }
  ...
  Future<void> _getCourses() async {
    try {
      ...
      if (enableCuratedProgram) {
        categories.add(PrimaryCategory.curatedProgram);
      }

      _courses = await Provider.of<LearnRepository>(context, listen: false)
          .getCourses(1, widget.searchKeyword, categories, [], [],
              isModerated: false);
final otherCourses =
          await Provider.of<LearnRepository>(context, listen: false).getCourses(
              1,
              '',
              [
                EnglishLang.course.toLowerCase(),
              ],
              [],
              [],
              isModerated: true);

      if (otherCourses != null && otherCourses.length > 0) {
        _courses.addAll(otherCourses);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: DefaultTabController(
          length: AssistantTab.items(context: context).length,
          child: SafeArea(
                ...
                SliverPersistentHeader(
                  delegate: SilverAppBarDelegate(
                    TabBar(
                      ...
                      tabs: [
                        for (var tabItem
                            in AssistantTab.items(context: context))
                          Container(
                            // width: 85,
